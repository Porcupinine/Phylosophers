...
/*   Updated: 2023/10/26 23:06:01 by laura         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static void	copy_data(t_philos_data *philos_data, const int count)
{
	...
	philos_data->philos[count].start_mut = philos_data->start_mut;
	philos_data->philos[count].forks_state = philos_data->forks_state;
	philos_data->philos[count].error = philos_data->error;
}

static int	init_philo_mutex(t_philos_data *philos_data, int count)
{
...
static int	set_philo(t_philos_data *philos_data)
{
	...
	while (count <= philos_data->amount)
	{
		...
		if (init_philo_mutex(philos_data, count) == 1)
			return (1);
		philos_data->philos[count].my_funeral = false;
		philos_data->philos[count].last_meal = phi_time();
